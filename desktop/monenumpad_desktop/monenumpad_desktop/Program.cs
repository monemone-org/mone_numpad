...
namespace monenumpad_desktop
{
    class Program
    {
        ...
        static void print_dev(hid_device_info dev)
        {
            Console.WriteLine("Device Found\n  vid: 0x{0:X4} pid: 0x{1:X4}\n  path: {2}\n  serial_number: {3}",
                dev.vendor_id, dev.product_id, dev.path, dev.serial_number);
            Console.WriteLine("  Manufacturer: {0}", dev.manufacturer_string);
            Console.WriteLine("  Product:      {0}", dev.product_string);
            Console.WriteLine("  Release:      0x{0:X4}", dev.release_number);
            Console.WriteLine("  Interface:    {0}", dev.interface_number);
            Console.WriteLine("  Usage (page): 0x{0:X4} (0x{1:X4})", dev.usage, dev.usage_page);
            Console.WriteLine("\n");
        }
        ...
        static void Main(string[] args)
        {
            ...
            hid_device_info? dev_monenumpad = null;

            void hid_on_add_device(/*struct hid_device_info * */IntPtr dev_ptr)
            {
                dev_monenumpad = new hid_device_info(dev_ptr);
                print_dev(dev_monenumpad.Value);
            };

            IntPtr devs = HidAPI.hid_enumerate_ex(
                Maxmix.structs.Constants.MONENUMPAD_VENDOR_ID,
                Maxmix.structs.Constants.MONENUMPAD_PRODUCT_ID,
                Maxmix.structs.Constants.MONENUMPAD_USAGE_PAGE,
                Maxmix.structs.Constants.MONENUMPAD_USAGE,
                hid_on_add_device);
            ...
            for (int i = 0; i < 10 && !dev_monenumpad.HasValue; ++i)
            {
                Console.WriteLine("Waiting for Mone NumPad.");
                Thread.Sleep(1000);
            }

            if (!dev_monenumpad.HasValue)
            {
